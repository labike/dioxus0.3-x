...
struct Cli {
    // apoi -d or api --database-url <URL>
    #[clap(
        short,
        long,
...
async fn run () -> Result<()> {
    ...
    if let Some(command) = args.command {
        match command {
            Command::GenKey => {
                ...
                info!(
                    target: "uchat_server",
                    "set API_PRIVATE_KEY environment variable with the content of the key in order to use it");
                return Ok(());
            }
        }
    }
    ...
    info!(target: "uchat_server", database_url = args.database_url, "connecting database...");
