...
fn profile_id_to_url(id: &str) -> Url {
    ...
    app_url::domain_and(user_content::ROOT).join(user_content::IMAGES).unwrap().join(id).unwrap()
}
...
#[async_trait]
impl AuthorizatedApiRequest for GetMyProfile {
...
#[async_trait]
impl AuthorizatedApiRequest for UpdateProfile {
    ...
    async fn process_request(
        ...
        if let Update::Change(ref img) = payload.profile_image {
            ...
            // 修复更新时直接将用户图像的base64格式数据存入数据库
            payload.profile_image = Update::Change(id.to_string())
        }

        let query_prams = UpdateProfileParams {
            id: session.user_id,
        ...
        uchat_query::user::update_profile(&mut conn, query_prams)?;
        let profile_image_url = {
            let user = uchat_query::user::get(&mut conn, session.user_id)?;

            user.profile_image.as_ref().map(|id| {
                profile_id_to_url(id)
            })
        };
