...
pub fn new(
...

    {
        ...
        diesel::insert_into(web::table).values(
            &new_session
        ).on_conflict(
            (web::user_id, web::fingerprint)
        ).do_update().set(
            web::expires_at.eq(new_session.expires_at)
        ).get_result(conn)
    }
}

pub fn get(
    conn: &mut PgConnection, session_id: SessionId
) -> Result<Option<Session>, DieselError> {
    use crate::schema::web;

    web::table.filter(web::id.eq(session_id)).get_result(conn).optional()
}

pub fn find(
    conn: &mut PgConnection, user_id: UserId, fingerprint: Fingerprint
) -> Result<Session, DieselError> {
    use crate::schema::web;

    web::table.filter(web::user_id.eq(user_id))
